...
- Repeatable browser work can use the scripted browser task lane, which creates a durable workspace with `plan.md`, `final_script.py`, per-run stdout/stderr, screenshots, logs, and JSON reports. This is the Webwright-style route for reproducible browser proof; it reports `script_required` until a real task-specific script exists.
