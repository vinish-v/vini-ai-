...
class Message(ApiHandler):
    ...
    async def communicate(self, input: dict, request: Request):
        ...
        if request.content_type.startswith("multipart/form-data"):
            ...
            voice_mode = bool(input_data.get("voice_mode"))
            attachment_paths = []
        ...
        if voice_mode:
            system_message.append(
                "Voice conversation mode is active. Reply fast for spoken playback: "
                "answer directly in one or two complete short sentences, put the "
                "useful answer in the first sentence, avoid markdown and long reasoning, "
                "avoid lists unless the user explicitly asks for a list, and call the "
                "final response tool as soon as the spoken answer is clear."
            )
