# Vini AI — Frontend Modals (stacked `openModal/closeModal`)

This document covers the stacked modal system used by Vini AI’s frontend, implemented in:
...
Vini AI uses a custom `<x-component path="...">` tag that is populated by the component loader.
...
Vini AI can open modals from many places (sidebar, settings, message actions), and sometimes a modal opens another modal.
...
Use `.modal-floating` on the outer `.modal` when a modal should behave like a floating utility panel instead of a blocking dialog. This is for special live surfaces where the user should keep seeing and interacting with the chat or dashboard behind the panel. The Browser now primarily lives in the Canvas, Vini AI's right-side workspace for live surfaces, but its floating/windowed surface follows the same non-blocking modal contract.
