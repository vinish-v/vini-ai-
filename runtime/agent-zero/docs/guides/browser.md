...
Vini AI has a built-in Browser for real web pages.
...
When a page is read, Vini AI gets simple references such as `[link 1]`,
...
When Vini AI takes a Browser screenshot, the image is saved and shown in the
chat history.
...
- **Chrome Extensions:** Install a Chrome Web Store URL, create a new extension with Vini AI, or scan an extension with Vini AI.
...
The Docker browser is the default. It is a separate browser inside Vini AI's
Docker environment, and it is the browser shown in the live Browser surface.

Use Docker browser mode when you want a clean, separate browser that Vini AI
can show in the Canvas.

In normal Docker installs, the needed browser is already included. In local
development, Vini AI can install it the first time it is needed.

## Bring Your Own Browser

Bring Your Own Browser lets Vini AI use Chrome, Edge, or Chromium on your own
...
The first time Vini AI tries to operate that browser, Chrome shows an **Allow
remote debugging?** prompt. Click **Allow** if you trust the connected Vini AI
...
> instances you intend Vini AI to control.
...
Start with Vini AI's built-in Browser.
...
- **Browser says Playwright is missing:** Docker installs already include the browser. In local development, let Vini AI install it on first use or preinstall it with `PLAYWRIGHT_BROWSERS_PATH=tmp/playwright playwright install chromium`.
