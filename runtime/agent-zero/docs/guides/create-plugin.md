...
The fastest way to understand Vini AI plugins is to make one small enough to
hold in your head.
...
## Ask Vini AI To Build It

Open a new chat and give Vini AI a very specific plugin task:
...
Vini AI instance. For this example, the final plugin shape is:
...
After creating or changing a plugin, restart Vini AI so the Web UI extension
list is rebuilt.
...
4. Wait for Vini AI to keep working in the first chat.
...
| Code patterns | PASS with WARN | The store uses Vini AI's `createStore` pattern. The unread signal is chat activity, not a parsed message-author check. |
...
- [DeepWiki for Vini AI](https://deepwiki.com/agent0ai/agent-zero): source-linked architecture when you need it.
