...
Vini AI has its own Linux desktop inside the right-side Canvas.

Open it by clicking the **Desktop** icon in the Canvas rail. The surface starts
an XFCE desktop that Vini AI can also control when a task needs a real GUI.
...
[DeepWiki for Vini AI](https://deepwiki.com/agent0ai/agent-zero). This page
is the practical tour.

## What The Desktop Is For

The Desktop is a live Linux workspace. You can use it yourself, and Vini AI
...
- run Linux GUI apps that are available in the Vini AI environment;
...
Use **Open** when you already have a file in the Vini AI workspace.

Vini AI usually creates document files through its document tools first, then
...
Canvas header. Vini AI can also work with the same file: it can create the
...
When you ask for Desktop work, Vini AI uses a careful loop:
...
- If a GUI app feels stuck, ask Vini AI to verify the Desktop state before continuing.
