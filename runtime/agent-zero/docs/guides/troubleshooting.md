...
This page addresses frequently asked questions (FAQ) and provides troubleshooting steps for common issues encountered while using Vini AI.

## Frequently Asked Questions
**1. How do I ask Vini AI to work directly on my files or dirs?**
- Place the files/dirs in `/a0/usr`. Vini AI will be able to perform tasks on them.
...
**6. How do I integrate open-source models with Vini AI?**
...
**7. How can I make Vini AI retain memory between sessions?**
Use **Settings -> Backup & Restore** and avoid mapping the entire `/a0` directory. See [How to update Vini AI](../setup/installation.md#how-to-update-agent-zero).
...
If you are running a local development checkout, Vini AI can install the
...
- Join the Vini AI [Skool](https://www.skool.com/agent-zero) or [Discord](https://discord.gg/B8KZKNsPpj) community.
...
**13. Can Vini AI interact with external APIs or services (e.g., WhatsApp)?**
...
The recovery command only schedules the update. Restart the container or let Vini AI start again, then check `/exe/a0-self-update.log` and `/exe/a0-self-update-status.yaml` to see what happened.
...
* **Performance Issues:** If Vini AI is slow or unresponsive, it might be due to resource limitations, network latency, or the complexity of your prompts and tasks, especially when using local models.
