...
def _scheduler_task_to_sidebar_task(
    task_details: dict[str, Any],
    *,
    context_available: bool,
) -> dict[str, Any]:
    task_id = str(task_details.get("uuid") or task_details.get("context_id") or "")
    created_at = task_details.get("created_at") or Localization.get().serialize_datetime(
        datetime.fromtimestamp(0)
    )
    updated_at = task_details.get("updated_at") or created_at

    context_data: dict[str, Any] = {
        "id": task_details.get("context_id") or task_id,
        ...
        "running": task_details.get("state") == "running",
        ...
        "next_run": task_details.get("next_run"),
...
async def build_snapshot_from_request(*, request: StateRequestV1) -> SnapshotV1:
    ...
    for ctx in all_ctxs:
        ...
        if not is_task_context:
            ...
            if task_details:
                task_context_data = _scheduler_task_to_sidebar_task(
                    task_details,
                    context_available=True,
                )
                for preserved_context_key in (
                    "id",
                    "name",
                    "created_at",
                    "no",
                    ...
                    task_context_data.pop(preserved_context_key, None)
                context_data.update(task_context_data)
                processed_task_ids.add(str(task_details.get("uuid") or ctx.id))

            tasks.append(context_data)

        processed_contexts.add(ctx.id)

    for task_details in scheduler.serialize_all_tasks():
        task_id = str(task_details.get("uuid") or "")
        if not task_id or task_id in processed_task_ids:
            continue
        tasks.append(
            _scheduler_task_to_sidebar_task(
                task_details,
                context_available=bool(AgentContext.get(str(task_details.get("context_id") or task_id))),
            )
        )
