...
Use `code_execution_remote` only for shell-backed execution on the machine where A0 CLI is running. Shells, runtimes, and paths belong to the CLI host, not the Vini AI server or Docker container.

If the task belongs inside Vini AI's own runtime, use the normal server-side execution tool instead.
...
- Match the remote host shell syntax. A Windows CLI may need PowerShell syntax even when Vini AI runs on Linux.

## Failure Handling

- If no CLI is connected, ask the user to connect A0 CLI to this Vini AI instance.
