...
Do not use this skill for macOS, Windows, Xpra, Docker, browser-only tasks, or the internal Vini AI Desktop. If the backend is not Linux or does not advertise AT-SPI support, skip Linux structural actions and follow the generic host computer-use rules.
