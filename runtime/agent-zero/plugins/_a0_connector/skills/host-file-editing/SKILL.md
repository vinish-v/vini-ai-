...
Use `text_editor_remote` only for file work on the machine where A0 CLI is running. These paths and files belong to the CLI host, not the Vini AI server or Docker container.

If the task belongs inside Vini AI's own runtime, use the normal server-side file tools instead.
