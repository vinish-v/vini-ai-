...
Actions: `open`, `list`, `state`, `observe`, `validate_ref`, `set_active`, `navigate`, `back`, `forward`, `reload`, `content`, `detail`, `screenshot`, `click`, `hover`, `double_click`, `right_click`, `drag`, `type`, `submit`, `type_submit`, `scroll`, `evaluate`, `key_chord`, `mouse`, `wheel`, `keyboard`, `clipboard`, `set_viewport`, `select_option`, `set_checked`, `upload_file`, `multi`, `close`, `close_all`.
...
- `observe` returns a structured browser state summary: URL, title, focused element, forms, visible interactive refs, page errors, action history, and optional screenshot/content.
- `validate_ref` checks that a target ref still exists, is attached, visible, enabled, and editable where needed. The runtime also validates refs before click/type/form actions.
