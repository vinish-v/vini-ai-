...
class Browser(Tool):
    async def execute(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            await self._emit_viewer_action(action, result, browser_id, x=x, y=y, text=text, key=key, keys=keys, url=url)
    ...
    async def _emit_viewer_action(
        ...
        **inputs: Any,
        ...
        if action not in VIEWER_ACTIONS:
            return
        context_id = self._agent_context_id()
        if not context_id:
            return

        payload = self._viewer_action_payload(action, result, requested_browser_id, inputs)
        try:
            from plugins._browser.api.ws_browser import emit_browser_viewer_action

            await emit_browser_viewer_action(context_id, payload)
        except Exception as exc:
            PrintStyle.debug("Browser viewer action emit failed:", f"action={action}", f"error={exc}")

    def _viewer_action_payload(
        self,
        action: str,
        result: Any,
        ...
        state = result_dict.get("state") if isinstance(result_dict.get("state"), dict) else result_dict
        action_payload = result_dict.get("action") if isinstance(result_dict.get("action"), dict) else {}
        point = self._viewer_action_point(action, action_payload, inputs)
        label = self._viewer_action_label(action, inputs)
        ...
        return {
            ...
            "action": action,
            "label": label,
            "browser_id": self._browser_id_from_result(result) or requested_browser_id,
            "browser_context_id": state.get("context_id") if isinstance(state, dict) else "",
            "point": point,
            "url": state.get("currentUrl") if isinstance(state, dict) else "",
            "title": state.get("title") if isinstance(state, dict) else "",
            "at": time.time(),
        }

    @staticmethod
    def _viewer_action_point(action: str, action_payload: dict[str, Any], inputs: dict[str, Any]) -> dict[str, Any] | None:
        if isinstance(action_payload.get("point"), dict):
            point = action_payload["point"]
            return {
                "x": float(point.get("x") or 0),
                "y": float(point.get("y") or 0),
            }
        for key in ("to", "from"):
            if isinstance(action_payload.get(key), dict):
                point = action_payload[key]
                return {
                    "x": float(point.get("x") or 0),
                    "y": float(point.get("y") or 0),
                }
        if action in {"mouse", "wheel", "hover", "double_click", "right_click", "drag", "click"}:
            x = inputs.get("x")
            y = inputs.get("y")
            try:
    ...
    @staticmethod
    def _viewer_action_label(action: str, inputs: dict[str, Any]) -> str:
        if action in {"type", "type_submit", "keyboard"}:
            text = str(inputs.get("text") or "")
            key = str(inputs.get("key") or "")
            keys = inputs.get("keys") or []
            if text:
                suffix = " and submit" if action == "type_submit" else ""
                return f"Typed {len(text)} character(s){suffix}"
            if key:
                return f"Pressed {key}"
            if keys:
                return f"Pressed {'+'.join(str(item) for item in keys)}"
        if action == "click":
            return "Clicked"
