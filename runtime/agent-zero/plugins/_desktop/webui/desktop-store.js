...
const DESKTOP_START_MESSAGE = "Starting Vini AI Desktop environment";
const DESKTOP_RUNTIME_INSTALL_MESSAGE = "Installing Vini AI Desktop runtime dependencies. This can take a few minutes after an update.";
...
const model = {
  ...
  async restartDesktopSession() {
    ...
    const session = await this.ensureDesktopSession({
      ...
      restart: true,
      select: true,
      message: "Restarting Vini AI Desktop environment",
    });
    if (!session) {
  ...
  async openDesktopWhenRuntimeReady(showProgress = true) {
    ...
    while (response?.ok === false && this.isDesktopRuntimeInstalling(response)) {
      ...
      if (Date.now() - startedAt > DESKTOP_RUNTIME_INSTALL_TIMEOUT_MS) {
        return {
          ...response,
          error: "Vini AI Desktop runtime installation is still running. Please try again in a moment.",
        };
      }
