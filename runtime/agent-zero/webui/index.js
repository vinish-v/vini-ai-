...
async function setMessages(...params) {
  const result = await msgs.setMessages(...params);
  updateChatHeroState();
  return result;
}
...
function updateChatHeroState() {
  const chatHistoryEl = document.getElementById("chat-history");
  if (!chatHistoryEl) {
    document.body.classList.remove("chat-has-real-messages");
    return;
  }

  const hasRealMessages = Boolean(
    chatHistoryEl.querySelector(".message-user, .process-group, .message-error, .message-warning, .message-tool, .message-code-exe, .message-browser"),
  );
  document.body.classList.toggle("chat-has-real-messages", hasRealMessages);
}
globalThis.updateChatHeroState = updateChatHeroState;
...
function afterMessagesUpdate(logs) {
  try {
    window.dispatchEvent(
      new CustomEvent("vini:agent-logs", {
        detail: { logs: Array.isArray(logs) ? logs : [] },
      }),
    );
  } catch (_error) {
    // Event dispatch is best-effort for optional UI layers.
  }
...
globalThis.stopAgent = async function () {
  ...
  if (!ctxid) throw new Error("No active chat context to stop.");
  const result = await sendJsonData("/stop", { context: ctxid });
  if (typeof syncStore.sendStateRequest === "function") {
    syncStore.sendStateRequest({ forceFull: true }).catch((error) => {
      console.error("[index] stopAgent sync refresh failed:", error);
    });
  }
  return result;
};
