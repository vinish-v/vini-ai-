...
export function drawMessageResponse({
  ...
  if (isInitialViniGreeting(content)) {
    return { element: document.createDocumentFragment(), dontScroll: true };
  }
...
function isInitialViniGreeting(content) {
  const normalized = String(content ?? "")
    .replace(/\*/g, "")
    .replace(/\s+/g, " ")
    .trim()
    .toLowerCase();
  return normalized.includes("i'm vini ai")
    && normalized.includes("your ai assistant")
