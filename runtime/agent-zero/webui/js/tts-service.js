...
class TtsService extends EventTarget {
  ...
  async speakStream(id, text, finished = false, options = {}) {
    ...
    const streamOptions = {
      ...
      fast: !!options.fast,
      maxChunkLength: Number(options.maxChunkLength || 135),
    };
    ...
    if (
      ...
      this.ttsStream.allowPartial === streamOptions.allowPartial &&
      this.ttsStream.fast === streamOptions.fast &&
      this.ttsStream.maxChunkLength === streamOptions.maxChunkLength
    ) {
      return;
    }
    ...
    if (!this.ttsStream || this.ttsStream.id !== id) {
      this.ttsStream = {
        ...
        stopped: false,
        chunks: [],
        allowPartial: streamOptions.allowPartial,
        fast: streamOptions.fast,
        maxChunkLength: streamOptions.maxChunkLength,
      };
    } else {
      this.ttsStream.finished = finished;
      this.ttsStream.text = text;
    ...
    this.ttsStream.chunks = this.chunkText(cleanText, {
      maxChunkLength: this.ttsStream.maxChunkLength,
    });
  ...
  async speakChunk(text, waitForPrevious = false, terminator = null, options = {}) {
  ...
  async speakWithProvider(provider, text, waitForPrevious = false, terminator = null, options = {}) {
    ...
    if (Array.isArray(payload.audioParts)) {
      for (const part of payload.audioParts) {
        ...
        await sleep(options.fast ? 20 : 100);
      }
      return;
    }
